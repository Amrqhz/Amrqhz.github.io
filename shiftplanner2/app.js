...
function init() {
  ...
  // Init alarm system after everything else is set up
  if (window.AlarmSystem) window.AlarmSystem.init();
}
...
function bindShiftForm() {
  ...
  // alarm toggle live sub-label
  const alarmEl = document.getElementById('shiftAlarmEnabled');
  if (alarmEl) {
    alarmEl.addEventListener('change', () => updateAlarmToggleSub(alarmEl.checked));
  }
}

function updateAlarmToggleSub(on) {
  const sub = document.getElementById('alarmToggleSub');
  if (!sub) return;
  sub.textContent = on ? '🔔 آلارم فعال است' : 'خاموش';
  sub.style.color = on ? 'var(--special-text, #a06010)' : '';
}
...
function resetShiftForm() {
  ...
  // Reset alarm toggle to global setting default
  const alarmEl = document.getElementById('shiftAlarmEnabled');
  if (alarmEl) {
    alarmEl.checked = settings.alarmEnabled || false;
    updateAlarmToggleSub(alarmEl.checked);
  }
  updateShiftTypeBadge();
}
...
function loadShiftIntoForm(id) {
  ...
  const alarmEl = document.getElementById('shiftAlarmEnabled');
  if (alarmEl) {
    alarmEl.checked = s.alarm !== undefined ? !!s.alarm : (settings.alarmEnabled || false);
    updateAlarmToggleSub(alarmEl.checked);
  }
...
function saveShiftFromForm() {
  ...
  const wantsAlarm = document.getElementById('shiftAlarmEnabled')?.checked ?? false;
  ...
  if (editId) {
    ...
    if (s) Object.assign(s, { pharmacyId, start, end, note, isNight, isHoliday: isHoliday || fridayHol, alarm: wantsAlarm });
    showToast('شیفت به‌روزرسانی شد');
  } else {
    shifts.push({ id: S.uid(), dateKey: selectedDateKey, pharmacyId, start, end, note,
                  isNight, isHoliday: isHoliday || fridayHol, alarm: wantsAlarm });
    showToast('شیفت ثبت شد');
  }
  S.saveShifts(shifts);

  // Rebuild alarms after every shift change
  if (window.AlarmSystem) {
    window.AlarmSystem.rebuildAlarms(shifts, pharmacies, settings);
    window.AlarmSystem.rescheduleAll();
  }
...
function bindAlarmSettings() {
  const globalToggle = document.getElementById('globalAlarmEnabled');
  if (!globalToggle) return;

  // Restore saved state
  globalToggle.checked = settings.alarmEnabled || false;
  updateNotifPermUI();
  ...
  globalToggle.addEventListener('change', async () => {
    ...
    if (enabled) {
      ...
      window.AlarmSystem.requestPermission((result) => {
        if (result === 'denied') {
          globalToggle.checked = false;
          showToast('مجوز اعلان رد شد — لطفاً از تنظیمات مرورگر فعال کنید');
          return;
        }
        ...
        window.AlarmSystem.rebuildAlarms(shifts, pharmacies, settings);
        window.AlarmSystem.rescheduleAll();
        showToast('آلارم شیفت‌ها فعال شد 🔔');
        updateNotifPermUI();
      });
    } else {
      ...
      window.AlarmSystem.rebuildAlarms(shifts, pharmacies, settings);
      window.AlarmSystem.rescheduleAll();
      showToast('آلارم شیفت‌ها خاموش شد');
      updateNotifPermUI();
    }
  });
  ...
  document.getElementById('requestNotifPermBtn')?.addEventListener('click', () => {
    window.AlarmSystem.requestPermission((result) => {
      updateNotifPermUI();
      if (result === 'granted') showToast('مجوز اعلان داده شد ✓');
      else showToast('مجوز اعلان رد شد');
    });
  });
  ...
  document.getElementById('testChimeBtn')?.addEventListener('click', () => {
    ...
    showToast('🎵 آزمایش صدای آلارم');
  });
}

function updateNotifPermUI() {
  const row  = document.getElementById('notifPermissionRow');
  const lbl  = document.getElementById('notifPermLabel');
  const desc = document.getElementById('notifPermDesc');
  if (!row) return;

  if (!('Notification' in window)) {
    row.style.display = '';
    if (lbl)  lbl.textContent  = 'اعلان پشتیبانی نمی‌شود';
    if (desc) desc.textContent = 'مرورگر شما از Notification API پشتیبانی نمی‌کند';
    document.getElementById('requestNotifPermBtn').style.display = 'none';
    return;
  }
  ...
  if (perm === 'granted') {
    ...
    row.style.display = '';
    if (perm === 'denied') {
      if (lbl)  lbl.textContent  = 'مجوز رد شده';
      if (desc) desc.textContent = 'در تنظیمات مرورگر دسترسی را فعال کنید';
      document.getElementById('requestNotifPermBtn').style.display = 'none';
    } else {
      if (lbl)  lbl.textContent  = 'مجوز اعلان سیستم';
      if (desc) desc.textContent = 'برای نمایش نوتیفیکیشن حتی زمانی که اپ بسته است';
      document.getElementById('requestNotifPermBtn').style.display = '';
    }
  }
}
...
function initThemePicker() {
  // Apply stored theme on init (already done by inline script but also sync UI)
  applyTheme(currentTheme, false);

  // Bind theme card buttons
